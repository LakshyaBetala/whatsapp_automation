...
- **Tally integration is TDL-free** (inline-TDL collection queries only; the `*.tdl` files in the repo root are dead). Hard-won facts encoded in `tally_agent/tally_xml.py`, all verified against a live TallyPrime: `SVCURRENTCOMPANY` is mandatory with >1 company open; ledger `ClosingBalance` is **negative when the customer owes** (parse flips the sign); voucher collections **ignore `SVFROMDATE`/`SVTODATE` over HTTP** and always return the active FY — hence sync sends the whole year and the backend dedups (sales via `bills.tally_voucher_number`, receipts via the `tally_receipts` table from `migrations/005_tally_receipts.sql`); responses arrive as UTF-16-BOM *or* UTF-8 depending on query type (`sanitize_xml` is BOM-aware — don't "simplify" it); debtors live in nested street/route subgroups under Sundry Debtors, so the group tree must be walked.

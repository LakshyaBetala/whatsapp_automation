...
VERSIONS = [
    ("1.9.0", "See your cash coming in, and what ASVA brought back",
     "Two things that make ASVA a cash-flow tool, not just reminders. A weekly Cash Coming In view shows how much is likely to arrive: what customers have promised plus what is due this week. And a monthly Recovery Report Card shows, in plain numbers, how much ASVA helped bring back this month versus last. Ask any time on WhatsApp by sending CASH or CARD, and the report card also arrives on its own at month-end. WhatsApp is steadier too: it now rides out small hiccups instead of logging out and asking for a new QR, so it needs far fewer re-scans. And the app no longer trips up if two customers were saved with the same phone number: a payment goes to whichever party actually owes money, the Payments tab always loads, and a leftover or test party always deletes cleanly."),
